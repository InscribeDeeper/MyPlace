(function($) {
    ...
    //     let address = $('#address').html() + ' Hoboken, NJ 07030'; // 解析地址
    //     geocoder.geocode({'address': address}, (results, status) => {
    //         if (status == 'OK') {
    //             // Display map of the restaurant with a marker
    //             map.setCenter(results[0].geometry.location); // 更改地图中心
    ...
    $('.favbtn').on('click', function(event) { // click 按钮
    ...
    $('.reportbtn').on('click', function(event) {
        ...
        if (userId) {
            var requestConfig = {
                method: "POST",
                url: '/api/report/' + revId + '/' + userId + '/' + restId, // report btn 对应着 几个id 都需要标记
                contentType: 'application/json',
                data: JSON.stringify({
                    rid: revId,
                    uid: userId,
    ...
    $('.likebtn').on('click', function(event) {
        ...
        if (userId) {
            ...
            $.ajax(requestConfig).then(function(responseMessage) { // ajax能够访问 post 对应的数据 到数据库. 只是这里的数据库接口用的是 api/like/ 
